...
import marketArtifact from '../../build/contracts/PaymentSplitter.json'
...
const App = {
  ...
  hashIt: function () {
    const self = this; 
    this.setHashStatus('Sending funds...please wait');
    const productPrice = parseInt(document.getElementById('productPrice').value)
    let market; 
    Market.deployed().then(function (instance){
      market = instance
      return market.toHash( { from: account , value: productPrice})
    }).then(function(res) {
      self.setHashStatus('Funds Sent ')
      console.log(res) 
    }).catch(function (e) {
      console.log(e)
